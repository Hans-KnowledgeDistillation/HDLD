...
def cat_mask1(t, mask1, mask2, mask3, mask4, mask5, mask6, mask7, mask8, mask9, not_mask):
    ...
    t8 = (t * mask8).sum(dim=1, keepdims=True)
    t9 = (t * mask9).sum(dim=1, keepdims=True)
    tn = (t * not_mask).sum(1, keepdims=True)
    rt = torch.cat([t1, t2, t3, t4, t5, t6, t7, t8, t9, tn], dim=1)
    return rt
...
def hc_loss1(logits_student_in, logits_teacher_in, target, alpha, beta, temperature, logit_stand=False):
    ...
    s_mask_9 = top_mask(y_t, 9).int() - top_mask(y_t, 8).int()
    s_mask_8 = top_mask(y_t, 8).int() - top_mask(y_t, 7).int()
    ...
    s_mask = top_mask(y_t, 9)
    not_s_mask = top_not_mask(y_t, 9)
    pred_student = cat_mask1(pred_student, s_mask_1, s_mask_2, s_mask_3, s_mask_4, s_mask_5, s_mask_6, s_mask_7, s_mask_8, s_mask_9, not_s_mask)
    pred_teacher = cat_mask1(pred_teacher, s_mask_1, s_mask_2, s_mask_3, s_mask_4, s_mask_5, s_mask_6, s_mask_7, s_mask_8, s_mask_9, not_s_mask)
...
def hc_loss(logits_student_in, logits_teacher_in, logits_teacher2_in, target, alpha, beta, temperature, logit_stand=False):
    y_s = normalize(logits_student_in) if logit_stand else logits_student_in
    y_t = normalize(logits_teacher_in) if logit_stand else logits_teacher_in
    y_t2 = normalize(logits_teacher2_in) if logit_stand else logits_teacher2_in

    pred_student = F.softmax(y_s / temperature, dim=1)
    pred_teacher = F.softmax(y_t / temperature, dim=1)
    #s_mask_t = _get_gt_mask(y_s, target)

    s_mask_5 = top_mask(y_t2, 5).int() - top_mask(y_t2, 4).int()
    s_mask_4 = top_mask(y_t2, 4).int() - top_mask(y_t2, 3).int()
    s_mask_3 = top_mask(y_t2, 3).int() - top_mask(y_t2, 2).int()
    s_mask_2 = top_mask(y_t2, 2).int() - top_mask(y_t2, 1).int()
    s_mask_1 = top_mask(y_t2,1).int()
    s_mask = top_mask(y_t2, 5)
    not_s_mask = top_not_mask(y_t2, 5)
...
class HCKD(Distiller1):
    def __init__(self, student, teacher, teacher2, cfg):
        super(HCKD, self).__init__(student, teacher, teacher2)
