...
def mdkd_loss1(logits_student_in, logits_teacher_in, target, alpha, beta, temperature, reduce=True, logit_stand=False):
    ...
    if reduce:
        consistency_loss = (((teacher_matrix - student_matrix) ** 2).sum() * 0.1 + (
                (teacher_matrix1 - student_matrix1) ** 2).sum() * 0.8) / class_num
    else:
        consistency_loss = (((teacher_matrix - student_matrix) ** 2) * 0.1 + (
                (teacher_matrix1 - student_matrix1) ** 2) * 0.8) / class_num
    ...
    if reduce:
        ...
        consistency_loss1 = (((teacher_matrix2 - student_matrix2) ** 2) * 0.1 + (
                (teacher_matrix3 - student_matrix3) ** 2) * 0.8) / class_num
    return tckd_loss * alpha + nckd_loss * beta + consistency_loss + consistency_loss1/target.shape[0]
...
def cc_loss(logits_student_in, logits_teacher_in, target, temperature, reduce=True, logit_stand1=False):
    logits_student = normalize(logits_student_in) if logit_stand1 else logits_student_in
    logits_teacher = normalize(logits_teacher_in) if logit_stand1 else logits_teacher_in
    ...
    pred_student_o = F.log_softmax(logits_student / temperature - 1000 * gt_mask, dim=1)
    pred_teacher_o = F.log_softmax(logits_teacher / temperature - 1000 * gt_mask, dim=1)
...
def bc_loss(logits_student_in, logits_teacher_in, target, temperature, reduce=True, logit_stand1=False):
    logits_student = normalize(logits_student_in) if logit_stand1 else logits_student_in
    logits_teacher = normalize(logits_teacher_in) if logit_stand1 else logits_teacher_in
...
class MDKD(Distiller):
    ...
    def forward_train(self, image_weak, image_strong, target, **kwargs):
        ...
        loss_kd_weak = min(kwargs["epoch"] / self.warmup, 1.0) * (self.kd_loss_weight1 * ((mdkd_loss(
        ...
        ) * mask).mean()) + self.kd_loss_weight1 * ((mdkd_loss(
            logits_student_weak,
            logits_teacher_weak,
            target,
            self.alpha,
        ...
        loss_kd_strong = min(kwargs["epoch"] / self.warmup, 1.0) * (self.kd_loss_weight1 * mdkd_loss(
        #loss_kd_strong = self.kd_loss_weight * dkd_loss(
            ...
            self.temperature,
            logit_stand=self.logit_stand,
        ) + self.kd_loss_weight1 * mdkd_loss(
            logits_student_strong,
            logits_teacher_strong,
            target,
            self.alpha,
