...
def dkd_loss(logits_student_in, logits_teacher_in, target, alpha, beta, temperature, logit_stand=True):
    ...
    return (alpha + 1) * tckd_loss / 10 + (beta) * nckd_loss / 10


def cc_loss(logits_student_in, logits_teacher_in, target, temperature, reduce=True, logit_stand=True):
    logits_student = normalize(logits_student_in) if logit_stand else logits_student_in
    logits_teacher = normalize(logits_teacher_in) if logit_stand else logits_teacher_in
    ...
    if reduce:
        consistency_loss = (((teacher_matrix - student_matrix) ** 2).sum() * 0.2 + (
                    (teacher_matrix1 - student_matrix1) ** 2).sum() * 0.8) / class_num
    else:
        consistency_loss = (((teacher_matrix - student_matrix) ** 2) * 0.2 + (
                (teacher_matrix1 - student_matrix1) ** 2) * 0.8) / class_num
    return consistency_loss


def bc_loss(logits_student_in, logits_teacher_in, target, temperature, reduce=True,logit_stand=True):
    logits_student = normalize(logits_student_in) if logit_stand else logits_student_in
    logits_teacher = normalize(logits_teacher_in) if logit_stand else logits_teacher_in
...
class MDLKD(Distiller):
    def __init__(self, student, teacher, cfg):
        super(MDLKD, self).__init__(student, teacher)
