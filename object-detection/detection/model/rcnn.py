...
from mdistiller.distillers.MDKD import mdkd_loss, cc_loss, bc_loss, mdkd_loss1
from mdistiller.distillers.KD import kd_loss
from mdistiller.distillers.HDLD import hc_loss, hc_loss1
...
def rcnn_hdld_loss(stu_predictions, tea_predictions, gt_classes, alpha, beta, temperature):
    stu_logits, stu_bbox_offsets = stu_predictions
    tea_logits, tea_bbox_offsets = tea_predictions
    gt_classes = torch.cat(tuple(gt_classes), 0).reshape(-1)
    loss_hdld = hc_loss(stu_logits, tea_logits, gt_classes, alpha, beta, temperature)
    return {
        'loss_hdld': loss_hdld,
    }
def rcnn_mdkd_loss(stu_predictions, tea_predictions, gt_classes, alpha, beta, temperature):
    stu_logits, stu_bbox_offsets = stu_predictions
    tea_logits, tea_bbox_offsets = tea_predictions
    gt_classes = torch.cat(tuple(gt_classes), 0).reshape(-1)
    loss_mdkd = mdkd_loss(stu_logits, tea_logits, gt_classes, alpha, beta, temperature)
    loss_cc = cc_loss(stu_logits, tea_logits, gt_classes, temperature)
    loss_bc = bc_loss(stu_logits, tea_logits, gt_classes,temperature)
    loss_mdkd1 = mdkd_loss1(stu_logits, tea_logits, gt_classes, alpha, beta, temperature)
    return {
        'loss_mdkd': loss_mdkd,
        #'loss_mdkd1': loss_mdkd1,
        'loss_cc': loss_cc,
        'loss_bc': loss_bc,
    }

def rcnn_kd_loss(stu_predictions, tea_predictions, temperature):
    stu_logits, stu_bbox_offsets = stu_predictions
    tea_logits, tea_bbox_offsets = tea_predictions
    loss_kd = kd_loss(stu_logits, tea_logits, temperature)
    return {
        'loss_kd': loss_kd,
    }
# def rcnn_mlkd_loss(stu_predictions, tea_predictions, temperature):
#     stu_logits, stu_bbox_offsets = stu_predictions
#     tea_logits, tea_bbox_offsets = tea_predictions
#     loss_kd = kd_loss(stu_logits, tea_logits, temperature)
#     loss_cc = cc_loss(stu_logits, tea_logits, temperature)
#     loss_bc = bc_loss(stu_logits, tea_logits, temperature)
#     return {
#         'loss_kd': loss_kd,
#         'loss_cc': loss_cc,
#         'loss_bc': loss_bc,
...
class RCNNKD(nn.Module):
    ...
    def __init__(
        ...
        *,
        ...
        if self.kd_args.TYPE in ("ReviewKD", "ReviewDKD", "ReviewMDKD", "ReviewHDLD"):
    ...
    def forward(self, batched_inputs: Tuple[Dict[str, torch.Tensor]]):
        ...
        if self.kd_args.TYPE == "DKD":
        ...
        elif self.kd_args.TYPE == "HDLD":
            teacher_images = self.teacher_preprocess_image(batched_inputs)
            t_features = self.teacher.backbone(teacher_images.tensor)
            stu_predictions = self.forward_pure_roi_head(self.roi_heads, features, sampled_proposals)
            tea_predictions = self.forward_pure_roi_head(self.teacher.roi_heads, t_features, sampled_proposals)
            detector_losses.update(rcnn_hdld_loss(
                stu_predictions, tea_predictions, [x.gt_classes for x in sampled_proposals],
                1.6, 0.25,1.0))
        elif self.kd_args.TYPE == "KD":
            teacher_images = self.teacher_preprocess_image(batched_inputs)
            t_features = self.teacher.backbone(teacher_images.tensor)
            stu_predictions = self.forward_pure_roi_head(self.roi_heads, features, sampled_proposals)
            tea_predictions = self.forward_pure_roi_head(self.teacher.roi_heads, t_features, sampled_proposals)
            detector_losses.update(rcnn_kd_loss(
                stu_predictions, tea_predictions, 2))
        elif self.kd_args.TYPE == "MDKD":
            teacher_images = self.teacher_preprocess_image(batched_inputs)
            t_features = self.teacher.backbone(teacher_images.tensor)
            stu_predictions = self.forward_pure_roi_head(self.roi_heads, features, sampled_proposals)
            tea_predictions = self.forward_pure_roi_head(self.teacher.roi_heads, t_features, sampled_proposals)
            detector_losses.update(rcnn_mdkd_loss(
                stu_predictions, tea_predictions, [x.gt_classes for x in sampled_proposals],
                self.kd_args.DKD.ALPHA, self.kd_args.DKD.BETA, self.kd_args.DKD.T))
            ...
            s_features = self.kd_trans(s_features)
            losses['loss_reviewkd'] = hcl(s_features, t_features) * self.kd_args.REVIEWKD.LOSS_WEIGHT
        elif self.kd_args.TYPE == "ReviewMDKD":
            teacher_images = self.teacher_preprocess_image(batched_inputs)
            t_features = self.teacher.backbone(teacher_images.tensor)
            # dkd loss
            stu_predictions = self.forward_pure_roi_head(self.roi_heads, features, sampled_proposals)
            ...
            s_features = self.kd_trans(s_features)
            losses['loss_reviewkd'] = hcl(s_features, t_features) * self.kd_args.REVIEWKD.LOSS_WEIGHT
        elif self.kd_args.TYPE == "ReviewHDLD":
            teacher_images = self.teacher_preprocess_image(batched_inputs)
            t_features = self.teacher.backbone(teacher_images.tensor)
            # dkd loss
            stu_predictions = self.forward_pure_roi_head(self.roi_heads, features, sampled_proposals)
