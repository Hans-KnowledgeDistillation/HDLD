...
def main_worker(gpu, ngpus_per_node, args):
    ...
    transform_train_enhance = MultipleApply([transform_train, transforms_train_strong])
    ...
    if args.dataset == 'cifar10':
        if args.loss_type in ['MLKD', 'MDKD']:
            train_dataset = IMBALANCECIFAR10(root='./data', imb_type=args.imb_type, imb_factor=args.imb_factor,
                                             ...
                                             transform=transform_train_enhance)
        else:
            train_dataset = IMBALANCECIFAR10(root='./data', imb_type=args.imb_type, imb_factor=args.imb_factor,
                                             rand_number=args.rand_number, train=True, download=True,
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        elif args.loss_type == 'LSKD':
            criterion = LSKDLoss(cls_num_list=cls_num_list, T=args.T, weight=per_cls_weights).cuda(args.gpu)
        # elif args.loss_type == 'DKD':
        #     criterion = DKDLoss(cls_num_list=cls_num_list, T=args.T, weight=per_cls_weights).cuda(args.gpu)
        elif args.loss_type == 'HDKD':
            criterion = HDLDLoss(cls_num_list=cls_num_list, T=args.T, weight=per_cls_weights).cuda(args.gpu)
        elif args.loss_type == 'HKD':
            criterion = HKDLoss(cls_num_list=cls_num_list, T=args.T, weight=per_cls_weights).cuda(args.gpu)
        # elif args.loss_type == 'MLKD':
        #     criterion = MLKDLoss(cls_num_list=cls_num_list, T=args.T, weight=per_cls_weights).cuda(args.gpu)
        elif args.loss_type == 'MDKD':
            criterion = MDKDLoss(cls_num_list=cls_num_list, T=args.T, weight=per_cls_weights).cuda(args.gpu)
        else:
            warnings.warn('Loss type is not listed')
            return

        # train for one epoch
        if args.loss_type == 'MDKD':
            train_strong(train_loader, teacher_model, student_model, criterion, optimizer, epoch, args, log_training,
                         tf_writer)
        elif args.loss_type == 'MLKD':
            train_strong(train_loader, teacher_model, student_model, criterion, optimizer, epoch, args, log_training,
                         tf_writer)
        ...
        if args.loss_type in ['MDKD', 'MLKD']:
            acc1 = validate_without_loss(val_loader, teacher_model, student_model, criterion, epoch, args, log_testing, tf_writer)
        else:
...
def train_strong(train_loader, teacher_model, student_model, criterion, optimizer, epoch, args, log, tf_writer):
    batch_time = AverageMeter('Time', ':6.3f')
    data_time = AverageMeter('Data', ':6.3f')
    losses = AverageMeter('Loss', ':.4e')
    top1 = AverageMeter('Acc@1', ':6.2f')
    ...
    for i, (input, target) in enumerate(train_loader):
        ...
        if args.gpu is not None:
            image_weak, image_strong = input
            image_weak = image_weak.cuda(args.gpu, non_blocking=True)
            image_strong = image_strong.cuda(args.gpu, non_blocking=True)
        target = target.cuda(args.gpu, non_blocking=True)

        # compute output
        with torch.no_grad():
            teacher_output_weak = teacher_model(image_weak)
            teacher_output_strong = teacher_model(image_strong)
        output_weak = student_model(image_weak)
        output_strong = student_model(image_strong)

        alpha = args.alpha

        if 'KD' in args.loss_type:
            loss = criterion(output_weak, output_strong, teacher_output_weak, teacher_output_strong, target, alpha)
        else:
            loss = criterion(output_weak, target)

        # measure accuracy and record loss
        acc1, acc5 = accuracy(output_weak, target, topk=(1, 5))
        losses.update(loss.item(), image_weak.size(0))
        top1.update(acc1[0], image_weak.size(0))
        top5.update(acc5[0], image_weak.size(0))
...
def validate(val_loader, teacher_model, student_model, criterion, epoch, args, log=None, tf_writer=None, flag='val'):
    ...
    with torch.no_grad():
        ...
        for i, (input, target) in enumerate(val_loader):
            ...
            if args.loss_type in ['KD', 'HKD', 'DKD', 'LSKD', 'HDKD']:
                loss, kd = criterion(output, teacher_output, target, alpha)
            else:
                loss = criterion(output, target)
...
def validate_without_loss(val_loader, teacher_model, student_model, criterion, epoch, args, log=None, tf_writer=None, flag='val'):
    batch_time = AverageMeter('Time', ':6.3f')
    #losses = AverageMeter('Loss', ':.4e')
    top1 = AverageMeter('Acc@1', ':6.2f')
    top5 = AverageMeter('Acc@5', ':6.2f')
    ...
    with torch.no_grad():
        ...
        for i, (input, target) in enumerate(val_loader):
            ...
            # alpha = 0
            #
            # if args.loss_type in ['KD', 'HKD', 'DKD']:
            #     loss, kd = criterion(output, teacher_output, target, alpha)
            # else:
            #     loss = criterion(output, target)
...
def load_network(network, args):
    if args.dataset == 'cifar10':
        if args.imb_factor == 0.01:
            ...
            save_path = 'teacher_checkpoint/cifar10_resnet32_CE_None_exp_0.02_0/ckpt.pth.tar'
        else:
            save_path = 'teacher_checkpoint/cifar10_resnet32_CE_None_exp_0.1_0/ckpt.pth.tar'
    else:
        if args.imb_factor == 0.01:
            save_path = 'teacher_checkpoint/cifar100_resnet32_CE_None_exp_0.01_0/ckpt.pth.tar'
        elif args.imb_factor == 0.02:
            save_path = 'teacher_checkpoint/cifar100_resnet32_CE_None_exp_0.02_0/ckpt.pth.tar'
        else:
            save_path = 'teacher_checkpoint/cifar100_resnet32_CE_None_exp_0.1_0/ckpt.pth.tar'
    if args.model_dir:
        save_path = os.path.join(args.model_dir, 'ckpt.pth.tar')
    print(save_path)
    # network = nn.DataParallel(network)
    network.load_state_dict({k.replace('module.',''):v for k, v in torch.load(save_path, map_location='cuda:0')['state_dict'].items()})
